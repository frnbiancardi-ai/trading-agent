...
- MetaTrader5 (TenTrade, conto demo)
...
## Stato progetto
- Branch attivo: feature/python-pure-strategy
- Fasi validate: 1–13
- Fase corrente: 14 (Python Pure Strategy Engine)
- Prossima: 15 (RSS News Sentiment)

## Regole fondamentali
- EXECUTION_MODE=shadow sempre di default
- Tutto da .env, zero magic numbers nel codice
- Risk engine = unico gate di approvazione trade
- Un file per fase in .orchestration/phase-prompts/
- STATE.md aggiornato dopo ogni micro-step
- PHASES.md è read-only per l'orchestrator

## Operatività intraday
- Timeframe: M15 (configurabile in .env)
- Simboli: array INTRADAY_SYMBOLS in .env (es. EURUSD,GBPUSD)
- Ciclo scheduler: ogni 5 minuti durante finestra operativa
- Finestra operativa: lun–ven 08:00–20:00 Europe/Rome
- Target: 5 decisioni finali/giorno (TRADE o NO_TRADE)
- Follow-up max: 120 minuti, una volta per opportunità

## Broker / MT5
- Broker: TenTrade demo
- Filling mode: ORDER_FILLING_RETURN (unico supportato su GBPUSD/EURUSD)
- Evitare ORDER_FILLING_IOC e ORDER_FILLING_FOK

## Convenzioni
- Non accedere mai direttamente a file .env
- Commit seguono COMMIT_CONVENTIONS.md
- Handoff segue HANDOFF_PROTOCOL.md
- Lingua commenti/log/rationale: italiano
- Test con pytest, mock Mt5Client per non dipendere da connessione reale