...
created: 2026-05-13
updated: 2026-05-13 (revision-loop iter 1 — Blocker #1 canonical alignment)
---
...
> **Revision 2026-05-13:** Blocker #1 alignment (canonical names ↔ Plan 10-01/03/04 granular split). Vedi §"Canonical ↔ Granular alias map" in fondo.
...
> **Revision Blocker #1:** righe canonical che corrispondono a un nome granular più descrittivo nel codice (Option A — alias documentale in questa map, niente wrapper Python) sono annotate `(alias di <granular>)` in colonna Behavior.

| Req | Behavior | Test Type | Automated Command | File Exists |
|-----|----------|-----------|-------------------|-------------|
| MCP-10 | `get_intermarket_context()` ritorna `usd_strength_bias`, `risk_on_off`, `jpy_safe_haven_flag` | unit | `pytest tests/test_mcp_handlers_macro.py::test_get_intermarket_context_envelope_enabled -x` | ❌ Wave 0 |
...
| MCP-10 | `ENABLE_INTERMARKET=false` → adjuster skipped, baseline parquet byte-identical (**D-10-D2 marquee gate canonical, umbrella Option B**) | regression | `pytest tests/test_phase10_no_phase5_drift.py::test_baseline_immutato_with_flag_off -x` | ❌ Wave 0 |
...
| MCP-13 | Tentative + All Day non-holiday events skipped (**canonical umbrella Option B**) | unit | `pytest tests/test_calendar_rss_client.py::test_skip_tentative_all_day -x` | ❌ Wave 0 |
...
| MCP-13 | MCP envelope shape success (**alias Option A di `test_economic_calendar_envelope_ok_success`**) | unit | `pytest tests/test_mcp_handlers_macro.py::test_economic_calendar_envelope_ok_success -x` | ❌ Wave 0 |
| MCP-13 | MCP envelope shape error (validation_failed) (**alias Option A di `test_economic_calendar_envelope_validation_failed_window_negative`**) | unit | `pytest tests/test_mcp_handlers_macro.py::test_economic_calendar_envelope_validation_failed_window_negative -x` | ❌ Wave 0 |

*Status colonna verra' allineata da planner in PLAN.md ↔ task IDs.*

---

## Canonical ↔ Granular alias map (Blocker #1 revision 2026-05-13)

Riconciliazione tra i nomi canonical previsti dalla validation matrix e i nomi
granular effettivamente prodotti in Wave 0 (Plan 10-01 Task 4 + Task 5). Le righe
sopra usano **già** il nome che esiste come simbolo Python; questa tabella documenta
l'intent dietro a ognuno per non perdere traceability futura.

| Canonical name (intent) | Strategia | Nome Python esistente | File |
|-------------------------|-----------|------------------------|------|
| `test_baseline_immutato_with_flag_off` | **Option B — umbrella vera in codice** | `test_baseline_immutato_with_flag_off` (Test 5, chiama internamente `test_dataset_writer_schema_v2_immutato` + `test_baseline_parquet_sha256_anchor` + asserisce default `ENABLE_INTERMARKET=false`) | `tests/test_phase10_no_phase5_drift.py` |
| `test_skip_tentative_all_day` | **Option B — umbrella vera in codice** | `test_skip_tentative_all_day` (chiama `_parse_event` per evento Tentative + evento Medium-All-Day, asserisce entrambi None) | `tests/test_calendar_rss_client.py` |
| `test_envelope_ok` | **Option A — alias documentale** (no wrapper Python) | `test_economic_calendar_envelope_ok_success` | `tests/test_mcp_handlers_macro.py` |
| `test_envelope_validation_failed` | **Option A — alias documentale** (no wrapper Python) | `test_economic_calendar_envelope_validation_failed_window_negative` | `tests/test_mcp_handlers_macro.py` |

**Verifica selettiva post-Wave 0:**
```bash
...
pytest tests/test_mcp_handlers_macro.py -k test_economic_calendar_envelope_ok_success --collect-only -q
pytest tests/test_mcp_handlers_macro.py -k test_economic_calendar_envelope_validation_failed_window_negative --collect-only -q
```
...
- [ ] `tests/test_calendar_rss_client.py` — stubs per MCP-13 RSS + cache + DST + holiday + fallback (incluso umbrella canonical `test_skip_tentative_all_day`)
- [ ] `tests/test_mcp_handlers_macro.py` — stubs per MCP-10/13 envelope + dispatch + validation
- [ ] `tests/test_phase10_no_phase5_drift.py` — D-10-C0 enforcement (Phase 5 schema-v2 immutability + parquet sha256 anchor + umbrella canonical `test_baseline_immutato_with_flag_off` D-10-D2 marquee gate)
...
- [ ] `tests/fixtures/ff_rss_dst_cross.xml` — DST boundary Mar 2nd Sun / Nov 1st Sun regression fixture (incluso Warning #7: ambiguous fall-back fold=0 EDT → UTC 05:30)
