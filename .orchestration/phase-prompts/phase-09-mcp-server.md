...
Attenzione: fase pesante in token. Prima di iniziare valuta handoff se hai già generato 4+ file in questa sessione.
...
Ogni tool registrato come `@server.tool()` (o equivalente nella versione installata) con `name`, `description`, `inputSchema` JSON. Output sempre `list[TextContent(type="text", text=json.dumps(result, default=str))]` o equivalente.
