...
class IntradayStrategy:
    ...
    def _analyze_technical(self, symbol: str, account_state: AccountState) -> TechnicalSetup:
        ...
        if None in (sma20, sma50, sma200, rsi_val, atr_val):
        ...
        # Try RSI_SMA strategy first (verified: 60.3% WR)
        if cfg.RSI_SMA_ENABLED:
            rsi_sma = self.identify_entry_rsi_sma(bars, indicators_snapshot)
            if rsi_sma.get("type") == "READY":
                return TechnicalSetup(
                    symbol=symbol, timeframe=timeframe, setup_type="READY",
                    direction=rsi_sma["direction"],
                    entry_price=rsi_sma.get("entry_price"),
                    stop_loss=rsi_sma.get("stop_loss"),
                    take_profit=rsi_sma.get("take_profit"),
                    confidence=0.8,
                    reason=rsi_sma.get("reason"),
    ...
    def identify_entry_rsi_sma(
        ...
        if current_hour not in [cfg.RSI_SMA_HOUR, cfg.RSI_SMA_HOUR + 1]:
            return {"type": "NONE", "direction": None, "reason": f"not_trade_hour={current_hour}"}
